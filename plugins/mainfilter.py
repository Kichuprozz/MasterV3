...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [
            [
                InlineKeyboardButton('ᴜᴘᴅᴀᴛᴇ', url='https://t.me/A2Z_BOTZ')
            ],
            [
        ...
        if not await db2.get_chat(message.chat.id):
            total=await client.get_chat_members_count(message.chat.id)
            await client.send_message(LOG_CHANNEL, script.LOG_TEXT_G.format(message.chat.title, message.chat.id, total, "Unknown"))       
            await db2.add_chat(message.chat.id, message.chat.title)
        return 
    if not await db2.is_user_exist(message.from_user.id):
        await db2.add_user(message.from_user.id, message.from_user.first_name)
...
async def channel_info(bot, message):
    ...
    if isinstance(CHANNELS, (int, str)):
        ...
        raise ValueError("ᴜɴᴇxᴘᴇᴄᴛᴇᴅ ᴛʏᴘᴇ ᴏғ ᴄʜᴀɴɴᴇʟ")
